...
if __name__ == "__main__":
    ...
    for cluster in clusters:
        for i in range(len(cluster)):
            ...
            asset1 = cluster[i]
            print(asset1)
            grouped = variable_dict[asset1].groupby('timestamp')['price'].agg(['max','min'])
            grouped['max_div_min'] = grouped['max'] / grouped['min']
            grouped['max_div_min'] = (grouped['max_div_min']-1)*100
            
            for critical_value, threshold in critical_values:
                filtered_data = grouped[grouped['max_div_min'] > critical_value]
            
                for timestamp, row in filtered_data.iterrows():
                    percentage_change1 = row['max_div_min']
                    
                    group = variable_dict[asset1][variable_dict[asset1].index == timestamp]
                    ...
                    if price_movement1 == 'Increased':
                        for asset2 in cluster:
                            if asset1 != asset2:
                                row_previous2 = variable_dict[asset2][variable_dict[asset2].index < timestamp]
                                if not row_previous2.empty:
                                    row_previous2 = row_previous2.tail(1)
                                else:
                                    row_previous2 = variable_dict[asset2].head(1)
                                    
                                for interval in intervals:
                                    start_time = pd.Timestamp(timestamp)
                                    end_time = start_time + pd.Timedelta(seconds=interval)
                                    selected_rows = variable_dict[asset2][(variable_dict[asset2].index >= start_time) & (variable_dict[asset2].index <= end_time)]
                                    
                                    selected_rows = pd.concat([row_previous2, selected_rows], ignore_index = False)
                                    ...
                                    if selected_rows.size > 1:
                                        ...
                                        exit_df = selected_rows[exit_condition]
                                        
                                        if not exit_df.empty:
                                            exit_index = exit_df.index[0]
        ...
        for interval in intervals:
            for cluster in clusters:
                for asset1 in cluster:
                    for asset2 in cluster:
                        if asset1 != asset2:
                            money = ((myDataFrames[(asset1,asset2,critical_value,interval)]['return']/100 + 1 ).prod() -1)*100
                            file_path = f"P:\Can Ceylan\HFT\_1 second intervals\Test_Results\{asset1}_{asset2}_{money:.2f}_{critical_value:.1f}_{threshold:.1f}_{interval:.0f}.csv"
